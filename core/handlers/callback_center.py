...
async def callback_factory(call: CallbackQuery, redis: Redis, state: FSMContext, db: PgSql, aio_http: SubServiceConn):
    """
    Общая функция для подключения коллбеков

    Для референса, вырвана из другого проекта
    """
    call_data = call.data
    tg_id = call.from_user.id

    # Раздел Подписок магазина
    if call_data == 'subs-shop-intro':
